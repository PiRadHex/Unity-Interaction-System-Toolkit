...
public class CameraController : MonoBehaviour
{
    ...
    public bool invertHorizontalMove = false;
    ...
    void Update()
    {
        ...
        currentZoom -= (Input.GetAxis("Mouse ScrollWheel") + Input.GetAxis("Vertical") * Time.deltaTime) * zoomSpeed;
        currentZoom = Mathf.Clamp(currentZoom, minZoom, maxZoom);

        // Adjust our camera's rotation around the player
        float yaw = Input.GetAxis("Horizontal") * yawSpeed * Time.deltaTime;
        currentYaw -= invertHorizontalMove ? -yaw : yaw;
    }
